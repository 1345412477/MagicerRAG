...
* **对话内图片提问（多模态）**：输入框左侧回形针可直接随消息上传图片，支持视觉的模型（如 gpt-4o / qwen-vl / glm-4v）可"看图提问"；当前模型不支持视觉时会友好提示，不会报错。

* **会话导出与只读分享**：一键导出 Markdown；生成带有效期、仅存哈希的只读分享链接。

* **安全与可观测**：模型 API Key 用 Fernet 加密落库、请求 ID / 安全响应头、操作审计、读多写少的并发控制。

* **自动化测试与 CI**：`pytest` 附带隔离存储夹具（临时 SQLite /upl 网 / 向量库）+ GitHub Actions。
...
```bash
cd /path/to/MagicerRAG
git pull origin main
docker compose up -d --build
```

对话内上传的图片持久化在 `./data/chat-uploads`（已挂载为容器内 `/app/static/chat-uploads`），重建容器不丢失。
...
| 分组                                       | 说明                                    |
| ---------------------------------------- | ------------------------------------- |
| `LLM_*`                                  | 推理模型：模型名 / API key /base\_url/ 温度     |
